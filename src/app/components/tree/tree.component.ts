...
textColorSelect.addEventListener('change', onColorSelect);
    ...
    nodeEnter.append("text")
        ...
        .on("mousemove", (event, d:any) =>
            div.style("left", (event.pageX)+ "px").style("top", (event.pageY) + "px")
                .html(
                "<table style='font-size: 18px; font-family: sans-serif; color:Black' >"+
   ...
   // zoom and pan behaviour added
    ...
    .on('zoom', (event) => {
        ...
        const leafNodeEnter = nodeEnter.filter((d: any) => !d.children && !d._children)
        .on("click", (event: any, d: any) => {
          // Check if the node is a leaf node
          if (!d.children && !d._children) {
            // Pass the leaf node data and cluster number to the new component
            const navigationExtras = {
              state: {
                leafNodeData: d.leafNodeData,
                clusterNumber: d.clusterNumber
              }
            };

            // Navigate to the new component
            this.router.navigate(['/leafnodedetails'], navigationExtras);
          }
        });
